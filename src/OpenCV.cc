...
OpenCV::Init(Handle<Object> target) {
  ...
  NODE_SET_METHOD(target, "readMat", ReadMat);
}  

NAN_METHOD(OpenCV::ReadMat) {
  NanEscapableScope();
  ...
  try{
    ...
    height = args[0]->Uint32Value();

    uint8_t *buf = (uint8_t *) Buffer::Data(args[2]->ToObject());

    tmp = cv::Mat(height, width, CV_8UC3, buf);
    cv::cvtColor(tmp, mat, CV_YUV2RGB);
